...
Check1:	bne $t0,0,Check2	# If m != 0 then go to next check else if m = 0 then go to return n + 1
	...
	j End
Check2:	blt $t0,0,End		# If m < 0 then go to return (just in case)
	
Mgt0:	addi $sp,$sp,-8		# Add space on stack for 4 words
...
RecEnd:	lw $ra,4($sp)		# Load $ra from stack
	addi $sp,$sp,8		# Move stack pointer back down
